...
//        moUnitRef = monthTable.child(Common.STR_Units);
    ...
    private BarData generateData(int year) {
        ...
        for (int i = 0; i <= monthUnits.size() - 1; i++) {
//            entries.add(new BarEntry(i, (float) (Math.random() * 70) + 30));
            entries.add(new BarEntry(i, Float.parseFloat(monthUnits.get(i))));
        }
        ...
        monthTable.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot monthDataSnapShot : dataSnapshot.getChildren()) {
                    ...
                    if (monthData != null) {
                        monthUnits.add(monthData.getMounits());
...
//                    Log.i("month", monthData.getMounits());
                ...
                if (months.size() <= 6) {
                    ...
                    for (int i = 0; i <= 5; i++) {
                        ...
                        fMonthunits.add(monthUnits.get(i));
                    }
                    for (int i = 6; i <= months.size() - 1; i++) {
                        lMonth.add(months.get(i));
                        lMonthunits.add(monthUnits.get(i));
                    }
                    for (int i = 0; i <= 1; i++) {
                        if (i == 0) {
                            monthUnits = fMonthunits;
                            list.add(generateData(Calendar.getInstance().get(Calendar.YEAR)));
                        } else {
                            monthUnits = lMonthunits;
                            list.add(generateData(Calendar.getInstance().get(Calendar.YEAR)));
                        }
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(BarChartActivity.this, (CharSequence) databaseError, Toast.LENGTH_SHORT).show();
            }
        });
    }
}
